...
MAX_CONTEXT_CHARS = get_setting("MAX_CONTEXT_CHARS", 6000)
RERANK_TOP_N = get_setting("RERANK_TOP_N", 3)
SEMANTIC_K = get_setting("SEMANTIC_K", 9)
ENABLE_NEIGHBOR_EXPANSION = get_setting("ENABLE_NEIGHBOR_EXPANSION", True)
...
REPORT_FILE = Path("reports") / "context_filter_all_questions_report.txt"
PREVIEW_CHARS = 900
...
def add_function_signature_report(lines):
    ...
    functions = [
        ...
        ("get_dynamic_retrieval_settings", get_dynamic_retrieval_settings),
        ("create_bm25_retriever", create_bm25_retriever),
    ]

    if get_query_type_label is not None:
        functions.append(("get_query_type_label", get_query_type_label))
...
def safe_int(value, default_value):
    # Convert config/dynamic values safely.
    try:
        return int(value)
...
def safe_bool(value, default_value):
    # Convert config/dynamic boolean values safely.
    if isinstance(value, bool):
        return value
...
def get_static_retrieval_settings():
    # Fallback only if dynamic context_filter settings fail.
    return {
        "mode": "static_fallback",
        "semantic_k": safe_int(SEMANTIC_K, 9),
        "bm25_k": safe_int(BM25_K, 9),
        "hybrid_final_k": safe_int(HYBRID_FINAL_K, 11),
        "rerank_input_k": safe_int(HYBRID_FINAL_K, 11),
        "rerank_top_n": safe_int(RERANK_TOP_N, 3),
        "final_top_n": None,
        "max_context_chars": safe_int(MAX_CONTEXT_CHARS, 6000),
        "neighbor_expansion": safe_bool(ENABLE_NEIGHBOR_EXPANSION, True),
    }


def get_question_dynamic_settings(question):
    # Get dynamic retrieval/rerank/context settings from retrieval.context_filter.
    # This makes this retrieval-only test use the same case-type policy.
    fallback_settings = get_static_retrieval_settings()

    try:
        dynamic_settings = dict(get_dynamic_retrieval_settings(question) or {})
    except Exception as error:
        fallback_settings["dynamic_error"] = f"{type(error).__name__}: {error}"
        return fallback_settings

    merged_settings = dict(fallback_settings)
    merged_settings.update(dynamic_settings)

    merged_settings["semantic_k"] = max(1, safe_int(merged_settings.get("semantic_k"), fallback_settings["semantic_k"]))
    merged_settings["bm25_k"] = max(1, safe_int(merged_settings.get("bm25_k"), fallback_settings["bm25_k"]))
    merged_settings["hybrid_final_k"] = max(1, safe_int(merged_settings.get("hybrid_final_k"), fallback_settings["hybrid_final_k"]))
    merged_settings["rerank_input_k"] = max(1, safe_int(merged_settings.get("rerank_input_k"), merged_settings["hybrid_final_k"]))
    merged_settings["rerank_top_n"] = max(1, safe_int(merged_settings.get("rerank_top_n"), fallback_settings["rerank_top_n"]))
    merged_settings["final_top_n"] = merged_settings.get("final_top_n")
    merged_settings["max_context_chars"] = max(1000, safe_int(merged_settings.get("max_context_chars"), fallback_settings["max_context_chars"]))
    merged_settings["neighbor_expansion"] = safe_bool(merged_settings.get("neighbor_expansion"), fallback_settings["neighbor_expansion"])

    # Rerank input should not exceed the hybrid pool created for this question.
    merged_settings["rerank_input_k"] = min(merged_settings["rerank_input_k"], merged_settings["hybrid_final_k"])
    merged_settings["rerank_top_n"] = min(merged_settings["rerank_top_n"], merged_settings["rerank_input_k"])

    return merged_settings


def get_max_dynamic_bm25_k():
    # BM25 retriever is created once, so make its pool large enough for the largest dynamic case.
    max_k = safe_int(BM25_K, 9)

    for question in TEST_QUESTIONS:
        dynamic_settings = get_question_dynamic_settings(question)
        max_k = max(max_k, safe_int(dynamic_settings.get("bm25_k"), max_k))

    return max_k


def add_call_parameter_report(lines, dynamic_settings=None, hybrid_count=None, rerank_input_count=None):
    # Show exact dynamic values this script passes to retrieval functions.
    dynamic_settings = dynamic_settings or get_static_retrieval_settings()

    semantic_k = safe_int(dynamic_settings.get("semantic_k"), SEMANTIC_K)
    bm25_k = safe_int(dynamic_settings.get("bm25_k"), BM25_K)
    hybrid_final_k = safe_int(dynamic_settings.get("hybrid_final_k"), HYBRID_FINAL_K)
    rerank_input_k = safe_int(dynamic_settings.get("rerank_input_k"), hybrid_final_k)
    rerank_top_n = safe_int(dynamic_settings.get("rerank_top_n"), RERANK_TOP_N)
    final_top_n = dynamic_settings.get("final_top_n")
    max_context_chars = safe_int(dynamic_settings.get("max_context_chars"), MAX_CONTEXT_CHARS)
    neighbor_expansion = safe_bool(dynamic_settings.get("neighbor_expansion"), ENABLE_NEIGHBOR_EXPANSION)

    lines.append(f"dynamic context mode               : {dynamic_settings.get('mode')}")
    if dynamic_settings.get("dynamic_error"):
        lines.append(f"dynamic settings error             : {dynamic_settings.get('dynamic_error')}")

    lines.append(f"create_bm25_retriever pool k       : {get_max_dynamic_bm25_k()}")
    ...
    lines.append(f"rerank_documents input_k           : {rerank_input_k}")
    lines.append(f"rerank_documents top_n             : {rerank_top_n}")

    if hybrid_count is None:
        lines.append("rerank_documents actual input docs : pending hybrid search")
    else:
        lines.append(f"rerank_documents actual input docs : {rerank_input_count} of {hybrid_count} hybrid docs")

    lines.append(f"select_final_context_docs top_n    : {final_top_n} / dynamic max by context_filter mode")
    lines.append(f"select_final_context_docs max_chars: {max_context_chars}")
    lines.append(f"select_final_context_docs neighbors: {neighbor_expansion}")
    lines.append("select_final_context_docs all_chunks: PASSED from loaded/cached chunks")
...
def add_chunk_flow_report(lines, semantic_docs, bm25_docs, hybrid_docs, reranked_docs, final_docs, max_rows=25):
    ...
    important_rerank_limit = min(7, len(reranked_docs or []))
...
def run_one_question(
    ...
    query_type_label = get_query_type_label(question) if get_query_type_label is not None else "N/A"
    ...
    lines.append(f"Query type detector label   : {query_type_label}")
    lines.append(f"Dynamic context mode        : {dynamic_settings.get('mode')}")
    ...
    lines.append("")
    add_call_parameter_report(lines, dynamic_settings=dynamic_settings)
    ...
    try:
        ...
        rerank_input_docs = hybrid_docs[:rerank_input_k]
        add_call_parameter_report(
            lines,
            dynamic_settings=dynamic_settings,
            hybrid_count=len(hybrid_docs),
            rerank_input_count=len(rerank_input_docs),
        )
        ...
        reranked_docs_with_scores = run_step(
            ...
            lambda: rerank_documents(
                query=question,
                documents=rerank_input_docs,
                reranker=reranker,
                top_n=min(len(rerank_input_docs), rerank_top_n),
                return_scores=True,
                debug=False,
            ),
        )
        ...
        add_docs_to_report(
            ...
            max_docs=min(10, hybrid_final_k),
        )
        ...
        add_docs_to_report(
            ...
            max_docs=min(10, len(reranked_docs)),
        )
        ...
        lines.append(f"Query type label : {query_type_label}")
        lines.append(f"Dynamic mode     : {dynamic_settings.get('mode')}")
        ...
        summary_rows.append(
            {
                ...
                "query_type_label": str(query_type_label),
                "dynamic_mode": str(dynamic_settings.get("mode")),
...
def add_batch_summary(lines, summary_rows):
    ...
    lines.append("Question | status | analyzer_mode | query_type | dynamic_mode | semantic | bm25 | hybrid | reranked | final")
    lines.append("-" * 120)

    for row in summary_rows:
        lines.append(
            f"{row['question']} | {row['status']} | {row['mode']} | {row.get('query_type_label', 'N/A')} | {row.get('dynamic_mode', 'N/A')} | "
...
def main():
    ...
    add_section(lines, "CONTEXT FILTER ALL QUESTIONS DEBUG TEST")
    ...
    dynamic_bm25_pool_k = get_max_dynamic_bm25_k()
    lines.append(f"Dynamic BM25 retriever pool k: {dynamic_bm25_pool_k}")
    ...
    try:
        ...
        print("")
        print("CONTEXT FILTER BATCH DEBUG DONE")
        print(f"Questions tested : {len(TEST_QUESTIONS)}")
        print(f"Report saved to  : {REPORT_FILE}")

        for row in summary_rows:
            print(f"- {row['status']} | final={row['final']} | analyzer={row['mode']} | dynamic={row.get('dynamic_mode', 'N/A')} | {row['question']}")
