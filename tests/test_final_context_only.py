...
from retrieval.context_filter import select_final_context_docs, detect_question_mode  # noqa: E402
from retrieval.semantic_retriever import format_semantic_query  # noqa: E402


DEFAULT_OUTPUT_PATH = PROJECT_ROOT / "reports" / "final_context_only_result.txt"
...
def add_config(lines, args, output_path):
    write_line(lines, "=" * 80)
    write_line(lines, "FINAL CONTEXT TEST: DOCS PASSED TO LLM ONLY")
    write_line(lines, "=" * 80)
    ...
    write_line(lines, f"Context top N       : {args.context_top_n or 'mode default'}")
...
def add_doc_result(lines, index, doc, stage_name):
    ...
    if stage_name == "CONTEXT":
        write_line(lines, f"   Ctx mode    : {metadata.get('context_mode', 'N/A')}")
        write_line(lines, f"   Ctx anchor  : {metadata.get('context_anchor_source', 'N/A')}")
        write_line(lines, f"   Ctx reason  : {metadata.get('context_anchor_reason', 'N/A')}")
        write_line(lines, f"   Ctx scope   : {metadata.get('context_confident_filter_scope', 'N/A')}")
        write_line(lines, f"   Keep reason : {metadata.get('context_keep_reason', 'N/A')}")
        write_line(lines, f"   Primary sc  : {metadata.get('primary_evidence_score', 'N/A')}")
        write_line(lines, f"   Direct sc   : {metadata.get('direct_window_score', 'N/A')}")
        write_line(lines, f"   Retrieval sc: {metadata.get('retrieval_agreement_score', 'N/A')}")
...
def run_single_query(lines, vectorstore, bm25_retriever, reranker, query_item, args):
    ...
    start_time = time.perf_counter()
    final_context_docs = select_final_context_docs(
    ...
    context_elapsed = time.perf_counter() - start_time
    detected_mode = detect_question_mode(raw_query)

    write_line(lines, f"Hybrid time        : {format_seconds(hybrid_elapsed)}")
    write_line(lines, f"Rerank time        : {format_seconds(rerank_elapsed)}")
    write_line(lines, f"Context time       : {format_seconds(context_elapsed)}")
    write_line(lines, f"Detected mode      : {detected_mode}")
    ...
    write_line(lines, f"Final context docs : {len(final_context_docs)}")

    add_stage_results(
        lines,
        "FINAL CONTEXT DOCS PASSED TO LLM",
        final_context_docs,
        args.context_top_n,
        "CONTEXT",
    )
    ...
    if final_context_docs:
        top_source = get_source_label(get_metadata(final_context_docs[0]))
    ...
    return {
        ...
        "context_results": len(final_context_docs),
        "mode": detected_mode,
        "hybrid_time": hybrid_elapsed,
        "rerank_time": rerank_elapsed,
        "context_time": context_elapsed,
...
def add_summary(lines, summaries):
    ...
    for item in summaries:
        write_line(
            lines,
            f"{item['id']:<6} "
            f"context={item['context_results']:<3} "
            f"mode={item['mode']:<12} "
            f"context_time={format_seconds(item['context_time']):>10} "
            f"top_context_source={item['top_source']}",
        )


def main():
    parser = argparse.ArgumentParser(description="Test final context docs passed to the LLM.")
    ...
    parser.add_argument("--context-top-n", type=int, default=0, help="Final context docs. 0 means mode default.")
    ...
    if args.context_top_n <= 0:
        args.context_top_n = None
    ...
    print("")
    print("FINAL CONTEXT ONLY TEST DONE")
